...
void main() {
  ...
  group('FirstRewardPopup / RewardBadgePopup Widget Tests', () {
    testWidgets('FirstRewardPopup.firstPlant renders correctly and responds to dismiss',
        (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: FirstRewardPopup.firstPlant(
      ...
      expect(
        find.textContaining('Monstera Hijau'),
        findsOneWidget,
      );
    ...
    testWidgets('FirstRewardPopup.timeCapsule renders correctly for time capsule badge',
        (tester) async {
      bool dismissed = false;
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: FirstRewardPopup.timeCapsule(
      ...
      expect(
        find.textContaining('Kaktus Koboi'),
        findsOneWidget,
      );
    ...
    testWidgets('FirstRewardPopup.fromBadge renders correctly for any generic badge',
        (tester) async {
      bool dismissed = false;
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: FirstRewardPopup.fromBadge(
              badge: testBadge,
              onDismiss: () => dismissed = true,
            ),
          ),
      ...
      expect(
        find.text('Menyiram tanaman tepat waktu selama 7 hari berturut-turut.'),
        findsOneWidget,
      );
  ...
  group('GrowthRepository.saveTimeCapsule Badge Unlocking Unit Tests', () {
    ...
    test('saveTimeCapsule unlocks time_capsule badge on first creation', () async {
      ...
      final badgeByIdRes = await badgeRepo.getBadgeById('time_capsule', userId: 1);
      ...
      final secondResultRes =
          await growthRepository.saveTimeCapsule(secondCapsule);
      ...
      expect(badges2.firstWhere((b) => b.id == 'time_capsule').isUnlocked, isTrue);
    });
  });
}
