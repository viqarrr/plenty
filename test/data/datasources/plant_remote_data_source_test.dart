...
import 'package:plenty/features/garden/domain/models/perenual/perenual_species_list_response.dart';
...
void main() {
  ...
  group('PlantRemoteDataSource Unit Tests', () {
    test(
      'PerenualSpeciesListResponse parses pagination metadata correctly',
      () {
        final json = {
          'data': [
            {
    ...
    test(
      'fetchSpeciesList returns parsed PerenualSpeciesModel list on HTTP 200',
      () async {
        ...
        expect(
          result[0].defaultImageUrl,
          'https://perenual.com/storage/fir.jpg',
        );
    ...
    test(
      'fetchSpeciesDetails returns parsed PerenualDetailModel on HTTP 200',
      () async {
        ...
        expect(
          ...
          9,
        ); // Average of 7-10 is 8.5 -> round to 9
        expect(
          result.defaultImageUrl,
          'https://perenual.com/storage/monstera.jpg',
        );
      },
    );

    test(
      'fetchSpeciesCareGuides returns parsed PerenualCareGuideModel list on HTTP 200',
      () async {
        ...
        expect(
          result.first.wateringAdvice,
          'Water when top 2 inches of soil are dry.',
        );
        expect(
          result.first.sunlightAdvice,
          'Provide medium to bright indirect light.',
        );
        expect(
          result.first.pruningAdvice,
    ...
    test(
      'fetchSpeciesList throws ServerFailure on HTTP 429 Quota Exceeded',
      () async {
        final client = MockClient((request) async {
    ...
    test(
      'fetchSpeciesList throws ServerFailure on HTTP 500 Internal Error',
      () async {
        final client = MockClient((request) async {
