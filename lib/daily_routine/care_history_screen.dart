...
final careHistoryProvider = FutureProvider.autoDispose<List<CareHistoryItem>>((
  ref,
) async {
  final user = await PreferenceHandler.getUser();
  final userId = (user?.id != null && user!.id! > 0) ? user.id.toString() : '1';
  final careRepo = ref.watch(careRepositoryProvider);
  return careRepo.getCareHistory(userId: userId);
});

/// Care Action History screen matching Plenty design language.
class CareHistoryScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final historyAsync = ref.watch(careHistoryProvider);
